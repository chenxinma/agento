    ...
    try:
        ...
        if request.stream:
            return StreamingResponse(
                stream_chat_response(request, conversation),
                media_type="text/event-stream"
            )
        else:
            return await generate_chat_response(request, conversation)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

async def generate_chat_response(request: ChatCompletionRequest, conversation: str) -> ChatCompletionResponse:
    # Run the agent
...
async def stream_chat_response(request: ChatCompletionRequest, conversation: str) -> AsyncGenerator[str, None]:
    response_id = f"chatcmpl-{uuid.uuid4().hex}"
    created_time = int(time.time())
